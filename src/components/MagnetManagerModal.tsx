...
import { Check, ListChecks, Search, Wand2, X } from 'lucide-react';
import {
  BoardZone,
  MagnetFontStyle,
  MagnetShape,
            ...
            <div className="p-4 space-y-3 border-b border-stone-100">
              <div className="flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="text-xs font-bold text-blue-700 hover:bg-blue-50 px-2 py-1 rounded-lg"
                >
                  {allVisibleSelected ? '표시 항목 해제' : '표시 항목 전체 선택'}
                </button>
              </div>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block text-sm font-bold text-stone-800 truncate">{token.title}</span>
                      <span className="block text-[11px] text-stone-500 truncate">
                        {token.subtitle || '직책 없음'} · {token.zoneId ? zoneNameMap.get(token.zoneId) || '알 수 없는 구역' : '자유 배치'}
                      </span>
                    </span>
