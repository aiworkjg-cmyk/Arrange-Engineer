...
export const UserScheduleHistoryDrawer: React.FC<Props> = ({
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-stone-900/50 backdrop-blur-xs">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-[240px_minmax(620px,1fr)_340px]">
        ...
        <main className="min-h-0 flex flex-col p-3 sm:p-4 overflow-hidden">
          ...
          <div className="flex-1 min-h-[520px] grid grid-cols-7 grid-rows-6 border-x border-b border-stone-200 rounded-b-xl overflow-hidden">{calendarDays.map((date) => {
            ...
            const openDayPopup = () => { setDayPopupDate(iso); setLayoutSnapshotId(''); };
            return <div key={iso} onDoubleClick={openDayPopup} title="더블클릭하여 등록 내역을 확인하고 새 항목 등록" className={`min-w-0 min-h-0 border-r border-b border-stone-100 p-1.5 overflow-hidden cursor-pointer flex flex-col ${inMonth ? 'bg-white' : 'bg-stone-50/70'}`}>
              ...
              {inMonth && <div className="grid grid-cols-2 gap-0.5 pt-0.5 shrink-0"><button type="button" onClick={(e) => { e.stopPropagation(); openDayPopup(); }} className="py-0.5 rounded bg-amber-50 text-[8px] font-bold text-amber-800 hover:bg-amber-100">+ 배치</button><button type="button" onClick={(e) => { e.stopPropagation(); openDayPopup(); }} className="py-0.5 rounded bg-violet-50 text-[8px] font-bold text-violet-700 hover:bg-violet-100">+ 일정</button></div>}
            </div>;
          })}</div>
        </main>
    ...
    {dayPopupDate && <div className="fixed inset-0 z-[65] flex items-center justify-center p-4 bg-stone-900/50" onMouseDown={(e) => { if (e.target === e.currentTarget) setDayPopupDate(null); }}>
      <div className="w-full max-w-6xl max-h-[92vh] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col" onMouseDown={(e) => e.stopPropagation()}>
        <header className="p-4 border-b flex items-center justify-between bg-stone-50"><div><h3 className="font-extrabold text-stone-900">{dayPopupDate} 배치 및 기사 일정</h3><p className="text-xs text-stone-500">상단에서 등록 내역을 확인하고, 하단에서 배치와 기사 일정을 각각 추가합니다.</p></div><button type="button" onClick={() => setDayPopupDate(null)} aria-label="날짜 등록창 닫기" className="p-2 rounded-full hover:bg-stone-200"><X className="w-4 h-4" /></button></header>
        <div className="overflow-y-auto custom-scrollbar">
          <section className="p-4 border-b-4 border-stone-200 bg-white">
            <div className="flex items-center justify-between mb-3"><div><h4 className="text-sm font-extrabold text-stone-900">등록된 내역</h4><p className="text-[11px] text-stone-500 mt-0.5">배치 {dayLayouts.length}개 · 기사 일정 {dayInstallerSchedules.length}개</p></div><span className="px-2.5 py-1 rounded-full bg-stone-100 text-[10px] font-bold text-stone-600">실시간 반영</span></div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
              <div className="rounded-xl border border-amber-200 bg-amber-50/70 p-3"><div className="flex items-center gap-2 mb-2"><FolderOpen className="w-4 h-4 text-amber-700" /><h5 className="text-xs font-extrabold text-amber-950">등록된 배치 <span className="text-amber-700">{dayLayouts.length}</span></h5></div>{dayLayouts.length ? <div className="space-y-2">{dayLayouts.map((schedule) => <button key={schedule.id} type="button" onDoubleClick={() => openSchedule(schedule)} onContextMenu={(e) => { e.preventDefault(); setContextMenu({ scheduleId: schedule.id, x: e.clientX, y: e.clientY }); }} className="w-full p-3 rounded-lg border border-amber-300 bg-white text-left hover:border-amber-500"><p className="text-xs font-extrabold text-amber-950">배치 · {schedule.snapshotName || schedule.title}</p><p className="mt-1 text-[10px] text-stone-500">더블클릭: 대시보드 미리보기 · 우클릭: 삭제</p></button>)}</div> : <p className="p-3 rounded-lg border border-dashed border-amber-300 text-xs text-amber-800 text-center">등록된 배치가 없습니다.</p>}</div>
              <div className="rounded-xl border border-violet-200 bg-violet-50/60 p-3"><div className="flex items-center gap-2 mb-2"><CalendarDays className="w-4 h-4 text-violet-700" /><h5 className="text-xs font-extrabold text-violet-950">등록된 기사 일정 <span className="text-violet-700">{dayInstallerSchedules.length}</span></h5></div>{dayInstallerSchedules.length ? <div className="space-y-2">{dayInstallerSchedules.map((schedule) => <button key={schedule.id} type="button" onDoubleClick={() => openSchedule(schedule)} onContextMenu={(e) => { e.preventDefault(); setContextMenu({ scheduleId: schedule.id, x: e.clientX, y: e.clientY }); }} className="w-full p-3 rounded-lg border border-violet-200 bg-white text-left hover:border-violet-400"><p className="text-xs font-extrabold text-stone-900">{schedule.title}</p><p className="mt-1 text-[10px] text-stone-500">{schedule.userName} · {schedule.timeRange} · {schedule.location}</p></button>)}</div> : <p className="p-3 rounded-lg border border-dashed border-violet-300 text-xs text-violet-700 text-center">등록된 기사 일정이 없습니다.</p>}</div>
            </div>
          </section>
          <section className="p-4 bg-stone-50">
            <div className="mb-3"><h4 className="text-sm font-extrabold text-stone-900">새 항목 등록</h4><p className="text-[11px] text-stone-500 mt-0.5">배치 등록과 기사 일정 등록은 서로 독립적으로 저장됩니다.</p></div>
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(260px,0.8fr)_minmax(420px,1.4fr)] gap-4 items-start">
              <div className="rounded-xl border-2 border-amber-200 bg-amber-50 p-4"><div className="flex items-center gap-2 mb-1"><FolderOpen className="w-4 h-4 text-amber-700" /><h5 className="text-sm font-extrabold text-amber-950">배치 등록</h5></div><p className="text-[11px] text-amber-800 mb-3">저장한 배치표를 이 날짜의 최상단에 등록합니다.</p>{snapshots.length ? <div className="space-y-2"><select value={layoutSnapshotId} onChange={(e) => setLayoutSnapshotId(e.target.value)} className={fieldClass}><option value="">저장 배치표 선택</option>{snapshots.map((snapshot) => <option key={snapshot.id} value={snapshot.id}>{snapshot.name}</option>)}</select><button type="button" disabled={!layoutSnapshotId} onClick={() => registerLayoutForDate(dayPopupDate)} className="w-full px-5 py-2.5 text-xs font-extrabold rounded-lg bg-amber-500 text-white disabled:opacity-40">배치 등록</button></div> : <p className="p-3 rounded-lg bg-white border border-amber-200 text-xs text-amber-900">먼저 상단의 배치표 저장/불러오기에서 이름을 정해 배치표를 저장해 주세요.</p>}</div>
              <div className="rounded-xl border-2 border-violet-200 bg-white overflow-hidden"><div className="px-4 pt-4"><div className="flex items-center gap-2"><CalendarDays className="w-4 h-4 text-violet-700" /><h5 className="text-sm font-extrabold text-violet-950">기사 일정 등록</h5></div><p className="text-[11px] text-violet-700 mt-1">기사, 기간, 현장 정보를 입력해 별도로 등록합니다.</p></div><ScheduleForm key={`day-${dayPopupDate}-${dayEvents.length}`} defaultDate={dayPopupDate} defaultInstallerId={defaultInstallerId} installers={installers} zones={zones} onCancel={() => setDayPopupDate(null)} onSubmit={(data) => onAddSchedule(data)} /></div>
            </div>
          </section>
        </div>
      </div>
